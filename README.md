...
| [`dbt-charts-dives`](./projects/dbt-charts-dives/) | Publishes every dbt Charts board in a dbt project as a live MotherDuck Dive during an ordinary `dbt build` — an `on-run-end` hook stages the boards, a Flight compiles them with dbt Charts, and the Dive runs the board's SQL in the viewer's browser. |
