...
def _fallback_scorer(predicted: str, gold: str) -> bool:
    ...
    pred_stripped = predicted.strip().strip('"').strip("'")
    gold_stripped = gold.strip().strip('"').strip("'")
...
def normalize_to_gold_format(predicted: str, gold: str) -> str:
    ...
    # Strip wrapping quotes off each element: an answer like ['B'] or 'B' is
    # semantically identical to B. Without this, the bracket strip leaves the
    # inner quotes (['B'] -> 'B') and the comparison fails on the stray quotes.
    gold_parts = [p.strip().strip("'\"") for p in gold_inner.split(",")]
    pred_parts = [p.strip().strip("'\"") for p in pred_inner.split(",")]
...
def score(
    ...
    if predicted_answer is None or predicted_answer.strip() == "":
        # An empty result is the correct answer when the gold is also empty / "" / N/A
        # (e.g. "which merchants were affected" → none). Match instead of failing.
        gold_norm = (gold_answer or "").strip().strip('"').lower()
        if gold_norm in _NA_VARIANTS:
            return ScoreResult(
                is_correct=True,
                correctness=Correctness.CORRECT,
                score=1.0,
                match_source="empty_match",
                reason=None,
                gold_answer=gold_answer,
