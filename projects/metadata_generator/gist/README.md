...
Implementation of [Automatic Metadata Extraction for Text-to-SQL](https://arxiv.org/abs/2505.19988) (arXiv:2505.19988) for MotherDuck databases.

## Files

Download these files to a directory:

- `metadata_generator.py` — Main script (single file, ~2100 lines)
...
# Set up environment
cp sample.env .env
# Edit .env with your MOTHERDUCK_TOKEN and OPENROUTER_API_KEY

# Step 1: Analyze query history and store patterns
uv run python metadata_generator.py -d mydb history myschema -x -t

# Step 2: Generate metadata with history context
uv run python metadata_generator.py -d mydb generate myschema --with-history -x
```

Two commands to go from raw database to rich semantic metadata.
...
**Flags:**
- `-x, --execute` — Store results in `metadata` schema tables
- `-t, --translate` — Translate query samples to natural language
...
| `query_use_cases` | Natural language translations of query samples |
...
**Flags:**
- `--with-history` — Use patterns from Step 1 for better descriptions
- `-x, --execute` — Apply COMMENT statements to MotherDuck
- `--model MODEL` — LLM model to use (default: google/gemini-3-flash-preview)
...
LLM generates semantic descriptions using profile data + history patterns.
...
- Python 3.11+
- MotherDuck account with `MOTHERDUCK_TOKEN`
- OpenRouter API key (`OPENROUTER_API_KEY`) for LLM descriptions
- MotherDuck Business plan with admin access (for query history)
