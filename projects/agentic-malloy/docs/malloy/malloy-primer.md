...
- **Rule/criteria tables with wildcards**: when selecting the rows that "apply to" a given value, a NULL scalar field (or empty list field) usually means "applies to ALL values". Match it as `(field = value or field is null)` for a scalar and `(len!number(col)=0 or list_contains!boolean(col, value))` for a list — bare equality silently drops the wildcard rows and under-counts. Apply the wildcard branch to EVERY criteria field.
