...
- PARAMETER FIDELITY. A parameter you declare to SCOPE a result (a filter / match-on argument) MUST change that result. Every filter parameter has to appear in a predicate that actually narrows the aggregated population — a \`where:\`, a join \`on:\`, or a \`pick ... when\` — that the measure is computed AFTER. Self-check every parameterized view: run it with two DIFFERENT values of each filter parameter; if the number does not move, the parameter is DEAD (declared in the signature but never wired), and a caller who scopes by it silently gets the GLOBAL aggregate. A parameter that only appears in the signature (never referenced in the body) is always a bug.
- VALIDATE A MATCH BY ROW-COUNT / CONSERVATION, NOT NON-EMPTINESS. Prove a fact×rule (or fact×criteria) match by conservation, not "it returned rows": every fact must match ≥1 rule (none silently unmatched — a wildcard/default row must catch the leftovers), and a counterfactual that overrides ONE match field must leave all OTHER matches identical — an override set to a fact's OWN current value must yield a ZERO delta. A total that is plausible-but-slightly-low is the signature of a dropped wildcard/default branch on one match field.
- NEVER CO-AGGREGATE TWO \`join_many\` RELATIONS IN ONE STAGE. A measure that combines aggregates from TWO different \`join_many\` relations on the same source — a difference, ratio, or two totals in the same \`aggregate:\` (e.g. a baseline vs a counterfactual, \`scenario_total - baseline_total\`, each summed over its own fanned join) — is SILENTLY WRONG: Malloy's per-join symmetric aggregates do not compose across two live fan-outs, so the query fans out over relationA×relationB and BOTH sums deflate (each is correct alone, wrong together). To compare/difference two fanned computations, pre-collapse EACH leg to one row per base key in its OWN single-\`join_many\` source (\`source: leg_by_key is <fact> extend { join_many: rel is ... } -> { group_by: <key>; aggregate: leg_total is rel.sum(...) }\`), then bring each back into the answering source via \`join_one\` on the key and combine those (join_one does not fan out). Verify: the same measure computed alone must equal its value when co-aggregated with the other.
